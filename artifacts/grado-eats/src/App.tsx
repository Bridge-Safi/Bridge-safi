...
interface UserProfile {
  ...
  cardNumber: string; cardExpiry: string; cardName: string;
}
...
const T = {
  fr: {
    ...
    trackingLabel:'Numéro de suivi', deliveryEta:'Livraison estimée dans 18–25 min',
    ...
    savedPayment:'Carte bancaire enregistrée', editProfile:'Modifier',
  ...
  en: {
    ...
    trackingLabel:'Tracking number', deliveryEta:'Estimated delivery in 18–25 min',
    newOrder:'New order',
    ...
    savedPayment:'Saved credit card', editProfile:'Edit',
  ...
  ar: {
    ...
    trackingLabel:'رقم التتبع', deliveryEta:'التوصيل المتوقع خلال 18–25 دقيقة',
    newOrder:'طلب جديد',
    autoFilled:'مُعبَّأ من ملفك الشخصي ✓',
    profileTitle:'ملفي الشخصي', profileSub:'معلوماتك المحفوظة',
    profileSave:'حفظ الملف الشخصي', profileSaved:'تم الحفظ ✓',
    savedPayment:'بطاقة بنكية محفوظة', editProfile:'تعديل',
  ...
  amz: {
    ...
    cardNumberPh:'1234 5678 9012 3456', cardExpiryPh:'MM/AA', cardCVVPh:'123', cardNamePh:'YOUSSEF ALAMI',
    payNow:'ⵙⵙⴼⵍⵍⴷ ⴷⵉⵖ 🔒',
    confirmWhatsApp:'ⵙⵙⴼⵍⵍⴷ ⵙ WhatsApp 🚀',
    successTitle:'ⵜⵜⵓⵙⵛⴷⵃ ⵜⴰⵖⵓⵍⵜ! 🎉', successSub:'ⵜⵜⵓⵙⵔⵖ ⵜⴰⵖⵓⵍⵜ ⵏⵏⴽ.',
    trackingLabel:'ⴰⵏⵓⵎⵔ ⵏ ⵓⵙⴽⵍⵙ', deliveryEta:'ⴰⵙⵍⵎⴷ ⵖ 18–25 ⵜⵉⵎⵉⵏⵉⵜⵉⵏ',
    newOrder:'ⵜⴰⵖⵓⵍⵜ ⵜⴰⵎⴰⵢⵏⵓⵜ',
    autoFilled:'ⵉⵜⵜⵓⵎⵍⴰ ⵙⴳ ⵓⵎⵍⵉ ⵏⵏⴽ ✓',
    profileTitle:'ⴰⵎⵍⵉ ⵏⵓ', profileSub:'ⵉⵙⴼⴰⵡⵏ ⵏⵏⴽ ⵉⵜⵜⵓⵙⵎⴷⵏ',
    profileSave:'ⵙⵎⴷ ⴰⵎⵍⵉ', profileSaved:'ⵜⵜⵓⵙⵎⴷ ✓',
    savedPayment:'ⵜⴰⴽⴰⵔⴷⵜ ⵉⵜⵜⵓⵙⵎⴷⵏ', editProfile:'ⵙⵏⴼⵍ',
...
interface MenuItem { id:number; emoji:string; category:string; names:Record<Lang,string>; price:number; safi?:boolean; }
interface CartItem extends MenuItem { qty:number; }

const MENU_ITEMS: MenuItem[] = [
  {id:1, emoji:'🍕',category:'pizza',  price:65,safi:true, names:{fr:'Pizza Fruits de Mer Safi',en:'Safi Seafood Pizza',ar:'بيتزا فواكه البحر آسفي',amz:'ⴱⵉⵜⵣⴰ ⵏ ⵡⴰⵍⵍⴰⵏ ⵏ ⵙⴰⴼⵉ'}},
  {id:2, emoji:'🍕',category:'pizza',  price:55, names:{fr:'Pizza Kefta Marocaine',en:'Moroccan Kefta Pizza',ar:'بيتزا الكفتة المغربية',amz:'ⴱⵉⵜⵣⴰ ⵏ ⴽⴼⵜⴰ'}},
  {id:3, emoji:'🍕',category:'pizza',  price:50, names:{fr:'Pizza 4 Fromages',en:'4 Cheese Pizza',ar:'بيتزا 4 أجبان',amz:'ⴱⵉⵜⵣⴰ 4 ⵉⴼⵔⵓⵎⴰⵊⵏ'}},
  {id:4, emoji:'🍕',category:'pizza',  price:45, names:{fr:'Pizza Végétarienne',en:'Vegetarian Pizza',ar:'بيتزا خضروات',amz:'ⴱⵉⵜⵣⴰ ⵏ ⵉⴷⴳⴰⵏ'}},
  {id:5, emoji:'🌯',category:'kebab',  price:35, names:{fr:'Sandwich Kefta Grillé',en:'Grilled Kefta Sandwich',ar:'ساندويش الكفتة المشوية',amz:'ⵙⴰⵏⴷⵡⵉⵜⵛ ⵏ ⴽⴼⵜⴰ'}},
  {id:6, emoji:'🌯',category:'kebab',  price:40, names:{fr:'Wrap Poulet Chermoula',en:'Chermoula Chicken Wrap',ar:'راب دجاج بالشرمولة',amz:'ⵡⵔⴰⴱ ⵏ ⴰⵢⵢⵓⵣ ⵛⵔⵎⵓⵍⴰ'}},
  {id:7, emoji:'🌯',category:'kebab',  price:35, names:{fr:'Panini Merguez',en:'Merguez Panini',ar:'باني مرقاز',amz:'ⴱⴰⵏⵉⵏⵉ ⵏ ⵎⵔⴳⵣ'}},
  {id:8, emoji:'🌯',category:'kebab',  price:75, names:{fr:'Assiette Kebab Mixte',en:'Mixed Kebab Plate',ar:'طبق كباب مشكل',amz:'ⵜⴰⵍⴼⵉⵙⵜ ⵏ ⴽⴱⴰⴱ'}},
  {id:9, emoji:'🦞',category:'seafood',price:55,safi:true, names:{fr:'Chraime de Safi',en:'Safi Chraime Fish',ar:'شرايم آسفي',amz:'ⵛⵔⴰⵉⵎ ⵏ ⵙⴰⴼⵉ'}},
  {id:10,emoji:'🦞',category:'seafood',price:70,safi:true, names:{fr:'Tajine de Sole',en:'Sole Fish Tajine',ar:'طاجين السمك المفلطح',amz:'ⵟⴰⵊⵉⵏ ⵏ ⵜⴰⵙⵓⵍⵜ'}},
  {id:11,emoji:'🦞',category:'seafood',price:65,safi:true, names:{fr:'Brochettes de Crevettes',en:'Shrimp Skewers',ar:'أسياخ الجمبري',amz:'ⴱⵔⵓⵛⵜ ⵏ ⵜⵖⵍⵍⴰ'}},
  {id:12,emoji:'🦞',category:'seafood',price:40,safi:true, names:{fr:'Sardines Grillées Safi',en:'Grilled Safi Sardines',ar:'السردين المشوي آسفي',amz:'ⵙⵔⴷⵉⵏ ⵖ ⵜⴼⵓⵏⴰⵙⵜ'}},
  {id:13,emoji:'🌮',category:'tacos',  price:40, names:{fr:'Tacos Poulet Fromage',en:'Chicken Cheese Tacos',ar:'تاكو دجاج وجبن',amz:'ⵜⴰⴽⵓⵙ ⵏ ⴰⵢⵢⵓⵣ'}},
  {id:14,emoji:'🌮',category:'tacos',  price:45, names:{fr:'Tacos Viande Hachée',en:'Ground Beef Tacos',ar:'تاكو اللحم المفروم',amz:'ⵜⴰⴽⵓⵙ ⵏ ⴰⴽⵙⵓⵎ'}},
  {id:15,emoji:'🌮',category:'tacos',  price:55,safi:true, names:{fr:'Tacos Crevettes Safi',en:'Safi Shrimp Tacos',ar:'تاكو جمبري آسفي',amz:'ⵜⴰⴽⵓⵙ ⵏ ⵜⵖⵍⵍⴰ ⵙⴰⴼⵉ'}},
  {id:16,emoji:'🌮',category:'tacos',  price:35, names:{fr:'Tacos Végétarien',en:'Vegetarian Tacos',ar:'تاكو خضروات',amz:'ⵜⴰⴽⵓⵙ ⵏ ⵉⴷⴳⴰⵏ'}},
  {id:17,emoji:'🍔',category:'burgers',price:55, names:{fr:'Burger Bridge Spécial',en:'Bridge Special Burger',ar:'برغر بريدج الخاص',amz:'ⴱⵓⵔⴳⵔ ⴱⵔⵉⴷⵊ'}},
  {id:18,emoji:'🍔',category:'burgers',price:65, names:{fr:'Burger Double Fromage',en:'Double Cheese Burger',ar:'برغر بجبن مزدوج',amz:'ⴱⵓⵔⴳⵔ ⵙⵉⵏ ⵉⴼⵔⵓⵎⴰⵊⵏ'}},
  {id:19,emoji:'🍔',category:'burgers',price:50, names:{fr:'Chicken Burger Chermoula',en:'Chermoula Chicken Burger',ar:'برغر دجاج شرمولة',amz:'ⴱⵓⵔⴳⵔ ⵏ ⴰⵢⵢⵓⵣ ⵛⵔⵎⵓⵍⴰ'}},
  {id:20,emoji:'🍔',category:'burgers',price:55, names:{fr:'Burger Kefta Marocain',en:'Moroccan Kefta Burger',ar:'برغر الكفتة المغربية',amz:'ⴱⵓⵔⴳⵔ ⵏ ⴽⴼⵜⴰ'}},
];
...
function Field({ label, value, onChange, placeholder, type='text', lang, error }:{
  label:string; value:string; onChange:(v:string)=>void;
  placeholder:string; type?:string; lang:Lang; error?:boolean;
}) {
  ...
  return (
    <div className="mb-4">
      ...
      <input
        ...
        style={{
          background: error ? '#FEF2F2' : '#F9F7F2',
          border:`2px solid ${error ? '#FCA5A5' : '#E5E1D8'}`,
          color:'#1A2F23',
...
function FoodCard({item,lang,onAdd,onView}:{item:MenuItem;lang:Lang;onAdd:(i:MenuItem)=>void;onView:(i:MenuItem)=>void}) {
  const t=T[lang]; const photo=CATEGORY_PHOTOS[item.category];
  return (
    <div onClick={()=>onView(item)}
      ...
      <div className="relative h-28 overflow-hidden">
        <img src={photo} alt={item.names[lang]} className="w-full h-full object-cover transition-transform duration-500 hover:scale-110" loading="lazy"/>
      ...
      <div className="p-2.5">
        <p className={`text-[11px] font-black leading-tight mb-2 line-clamp-2 ${fontClass(lang)}`} style={{color:'#1A2F23'}}>{item.names[lang]}</p>
        <div className="flex items-center justify-between gap-1">
          ...
          <button onClick={e=>{e.stopPropagation();onAdd(item);}}
            className="w-7 h-7 rounded-full flex items-center justify-center text-white font-black text-base transition-all active:scale-90"
            style={{background:'#4F46E5',flexShrink:0}}>+</button>
        </div>
      </div>
    </div>
  );
...
function QuickViewModal({item,lang,onClose,onAdd}:{item:MenuItem;lang:Lang;onClose:()=>void;onAdd:(i:MenuItem)=>void}) {
  const t=T[lang]; const isAR=lang==='ar'; const photo=CATEGORY_PHOTOS[item.category];
  return (
    <div className="fixed inset-0 z-50 flex items-end modal-overlay" style={{background:'rgba(10,30,20,0.6)',backdropFilter:'blur(4px)'}} onClick={onClose}>
      <div className="w-full max-w-md mx-auto rounded-t-3xl pb-8 modal-sheet" style={{background:'#FDFCF9'}} onClick={e=>e.stopPropagation()}>
        <div className="relative h-52 rounded-t-3xl overflow-hidden">
          ...
          {item.safi && <span className="absolute top-4 right-4 text-[10px] font-black px-2 py-1 rounded-full" style={{background:'#D9C5A0',color:'#065F46'}}>{t.safiExcl}</span>}
        </div>
        <div className="px-6 pt-5" style={{direction:isAR?'rtl':'ltr'}}>
          <p className={`text-xl font-black leading-snug mb-1 ${fontClass(lang)}`} style={{color:'#065F46'}}>{item.names[lang]}</p>
          <p className="text-2xl font-black mb-5" style={{color:'#B45309'}}>{item.price} MAD</p>
          <div className="flex gap-3">
            <button onClick={onClose} className="flex-1 py-3.5 rounded-2xl font-black text-sm border-2 transition-all active:scale-95" style={{borderColor:'#E5E1D8',color:'#6B7280',background:'white'}}>{t.close}</button>
            <button onClick={()=>{onAdd(item);onClose();}} className="flex-1 py-3.5 rounded-2xl font-black text-sm text-white transition-all active:scale-95" style={{background:'#4F46E5',boxShadow:'0 6px 20px rgba(79,70,229,0.35)'}}>
...
function ProfileModal({lang,profile,onSave,onClose}:{lang:Lang;profile:UserProfile;onSave:(p:UserProfile)=>void;onClose:()=>void}) {
  ...
  return (
    <div className="fixed inset-0 z-50 modal-overlay" style={{background:'rgba(10,30,20,0.55)',backdropFilter:'blur(6px)'}} onClick={onClose}>
      <div className="absolute right-0 top-0 bottom-0 w-full max-w-sm modal-sheet h-full overflow-y-auto"
        style={{background:'#FDFCF9',boxShadow:'-8px 0 40px rgba(0,0,0,0.15)'}} onClick={e=>e.stopPropagation()}>
        {/* Header */}
        <div className="sticky top-0 z-10 px-5 py-4 flex items-center justify-between" style={{background:'rgba(253,252,249,0.95)',backdropFilter:'blur(12px)',borderBottom:'1px solid #E5E1D8'}}>
          <div>
          ...
          <div className="rounded-2xl p-4 mb-5" style={{background:'#EEF2FF',border:'1px solid #C7D2FE'}}>
            ...
            {form.cardNumber && (
              <div className="rounded-2xl p-4 mb-4 relative overflow-hidden" style={{background:'linear-gradient(135deg,#065F46,#047857)',minHeight:100}}>
                ...
                <p className="text-white/60 text-[10px] font-bold mb-2">💳 BRIDGE EATS CARD</p>
                <p className="text-white font-black text-base tracking-widest mb-2">{fmtCard(form.cardNumber)||'•••• •••• •••• ••••'}</p>
                <div className="flex justify-between items-end">
                  <div>
                    <p className="text-white/50 text-[9px]">NAME</p>
                    <p className="text-white text-xs font-bold">{form.cardName||'—'}</p>
                  </div>
                  <div className="text-right">
                    <p className="text-white/50 text-[9px]">EXPIRES</p>
                    <p className="text-white text-xs font-bold">{form.cardExpiry||'—'}</p>
                  </div>
                </div>
              </div>
            )}
...
function CheckoutDrawer({cart,lang,onClose,onQty,profile,onClearCart}:{
  ...
  const [cardNum,setCardNum]=useState(profile.cardNumber?'•'.repeat(Math.min(profile.cardNumber.length,16)):'');
  ...
  const buildWaMsg=()=>{
    ...
    cart.forEach(i=>{msg+=`- ${i.names[lang]} x${i.qty} = ${i.price*i.qty} MAD\n`;});
    msg+=t.waMsgFooter(total,name.trim(),addr.trim(),phone.trim());
    return msg;
  };

  const goToPayment=()=>{
    if(!name.trim()||!addr.trim()||!phone.trim()){setErr(t.fillAll);return;}
    setErr(''); setStep('payment');
  };

  const confirmCash=()=>{
    window.open(`https://wa.me/212600000000?text=${encodeURIComponent(buildWaMsg())}`,'_blank');
    setStep('success');
  };

  const confirmCard=()=>{
    if(!cardCVV||cardCVV.length<3){setCardErr(t.fillAll);return;}
    setCardErr(''); setStep('success');
  };

  const handleSuccess=()=>{onClearCart();onClose();};

  const STEP_LABELS:Record<CheckoutStep,string>={
    cart:`🛒 ${t.cartTitle}`,
    form:`📋 ${t.checkoutTitle}`,
    payment:`💳 ${t.payModeTitle}`,
    card:`🔒 ${t.cardFormTitle}`,
    success:`✅ ${t.successTitle.replace(' 🎉','')}`,
  };
      ...
      <div className="w-full max-w-md mx-auto rounded-t-3xl modal-sheet" style={{background:'#FDFCF9',maxHeight:'92vh',display:'flex',flexDirection:'column'}} onClick={e=>e.stopPropagation()}>
        ...
        <div className="px-5 py-4 flex items-center justify-between flex-shrink-0" style={{borderBottom:'1px solid #E5E1D8'}}>
          <div className="flex items-center gap-2">
            {step!=='cart'&&step!=='success'&&(
              <button onClick={()=>setStep(step==='form'?'cart':step==='payment'?'form':'payment')}
                className="w-8 h-8 rounded-full flex items-center justify-center font-black" style={{background:'#F3F4F6',color:'#065F46',fontSize:14}}>←</button>
            )}
            <p className={`font-black text-sm ${fClass}`} style={{color:'#065F46'}}>{STEP_LABELS[step]}</p>
          </div>
          {step!=='success'&&<button onClick={onClose} className="w-8 h-8 rounded-full flex items-center justify-center font-black" style={{background:'#F3F4F6',color:'#6B7280',fontSize:16}}>✕</button>}
        </div>
            ...
            <div className="flex-1 overflow-y-auto px-5 py-3" style={{direction:isAR?'rtl':'ltr'}}>
              {cart.length===0?(
                ...
                <div key={item.id} className="flex items-center gap-3 py-3" style={{borderBottom:'1px solid #F3F4F6'}}>
                  <img src={CATEGORY_PHOTOS[item.category]} alt={item.names[lang]} className="w-12 h-12 rounded-xl object-cover flex-shrink-0"/>
        ...
        {step==='form'&&(
          <>
            <div className="flex-1 overflow-y-auto px-5 py-4" style={{direction:isAR?'rtl':'ltr'}}>
              ...
              {err&&<p className={`text-xs font-bold -mt-2 mb-3 ${fClass}`} style={{color:'#DC2626'}}>{err}</p>}
            </div>
            <div className="px-5 py-4 flex-shrink-0" style={{borderTop:'1px solid #E5E1D8'}}>
              <button onClick={goToPayment} className={`w-full py-4 rounded-2xl font-black text-sm text-white transition-all active:scale-95 ${fClass}`}
                style={{background:'linear-gradient(135deg,#065F46,#047857)',boxShadow:'0 6px 20px rgba(6,95,70,0.3)'}}>
                {t.continueBtn}
              </button>
            </div>
          ...
          <>
            <div className="flex-1 overflow-y-auto px-5 py-4">
              ...
              <button onClick={()=>setPayMethod('cash')}
                className="w-full flex items-center gap-4 p-4 rounded-2xl mb-3 transition-all active:scale-95 text-left"
                style={{background:payMethod==='cash'?'#F0FDF4':'#FDFCF9',border:`2px solid ${payMethod==='cash'?'#065F46':'#E5E1D8'}`}}>
                <div className="w-12 h-12 rounded-full flex items-center justify-center text-2xl flex-shrink-0" style={{background:payMethod==='cash'?'#D1FAE5':'#F0EDE7'}}>🤝</div>
                ...
                <div className="w-5 h-5 rounded-full border-2 flex-shrink-0 flex items-center justify-center"
                  style={{borderColor:payMethod==='cash'?'#065F46':'#D1D5DB',background:payMethod==='cash'?'#065F46':'transparent'}}>
              ...
              <button onClick={()=>setPayMethod('card')}
                className="w-full flex items-center gap-4 p-4 rounded-2xl mb-3 transition-all active:scale-95 text-left"
                style={{background:payMethod==='card'?'#EEF2FF':'#FDFCF9',border:`2px solid ${payMethod==='card'?'#4F46E5':'#E5E1D8'}`}}>
                <div className="w-12 h-12 rounded-full flex items-center justify-center text-2xl flex-shrink-0" style={{background:payMethod==='card'?'#E0E7FF':'#F0EDE7'}}>💳</div>
                <div className="flex-1 text-left">
              ...
              <div className="flex items-center gap-2 px-3 py-2 rounded-xl mt-2" style={{background:'#F9F7F2'}}>
                ...
                <p className="text-[10px]" style={{color:'#9CA3AF'}}>
                  {lang==='ar'?'جميع المدفوعات مشفرة وآمنة':lang==='amz'?'ⴰⵙⵙⴼⵍⵍⴷ ⴰⵎⵣⵡⴰⵔⵓ':'Paiement 100% sécurisé · SSL'}
                </p>
              </div>
            </div>
            ...
            <div className="px-5 py-4 flex-shrink-0" style={{borderTop:'1px solid #E5E1D8'}}>
              <button
                onClick={()=>{if(!payMethod)return;if(payMethod==='cash')confirmCash();else setStep('card');}}
                disabled={!payMethod}
                className={`w-full py-4 rounded-2xl font-black text-sm text-white transition-all active:scale-95 ${fClass}`}
                style={{
                  background:!payMethod?'#E5E1D8':payMethod==='cash'?'#25D366':'#4F46E5',
                  boxShadow:payMethod?'0 6px 20px rgba(0,0,0,0.2)':'none',
            ...
            <div className="flex-1 overflow-y-auto px-5 py-4" style={{direction:isAR?'rtl':'ltr'}}>
              ...
              <div className="rounded-2xl p-5 mb-5 relative overflow-hidden" style={{background:'linear-gradient(135deg,#065F46,#047857)',minHeight:120}}>
                ...
                <div className="flex justify-between items-start mb-4">
                  ...
                  <div className="flex gap-1">
                    ...
                    <div className="w-6 h-6 rounded-full bg-white/40 -ml-2"/>
                  </div>
                </div>
                <p className="text-white font-black text-base tracking-widest mb-3">
                  {cardNum?fmtCard(cardNum.replace(/•/g,'0')):'•••• •••• •••• ••••'}
                </p>
                <div className="flex justify-between items-end">
              ...
              {profile.cardNumber&&(
                <button onClick={()=>{setCardNum(profile.cardNumber);setCardExp(profile.cardExpiry);setCardName(profile.cardName);}}
                  ...
                  <span className="text-sm">✨</span>
                  <p className="text-[11px] font-bold" style={{color:'#4F46E5'}}>{t.autoFilled}</p>
                </button>
              )}
              ...
              {cardErr&&<p className={`text-xs font-bold -mt-2 mb-3 ${fClass}`} style={{color:'#DC2626'}}>{cardErr}</p>}
            </div>

            <div className="px-5 py-4 flex-shrink-0" style={{borderTop:'1px solid #E5E1D8'}}>
              <div className="flex items-center justify-center gap-2 mb-3">
                <span className="text-sm">🔒</span>
                <p className="text-[10px]" style={{color:'#9CA3AF'}}>256-bit SSL · PCI DSS Compliant</p>
              </div>
              <button onClick={confirmCard} className={`w-full py-4 rounded-2xl font-black text-sm text-white transition-all active:scale-95 ${fClass}`}
        ...
        {step==='success'&&(
          <div className="flex-1 flex flex-col items-center justify-center px-8 py-10 text-center">
            ...
            <div className="w-full rounded-2xl p-3 mb-6" style={{background:'#F9F7F2',border:'1px solid #E5E1D8'}}>
              {cart.map(i=>(
                <div key={i.id} className="flex justify-between text-xs py-0.5">
                  <span className={`font-medium truncate mr-2 ${fClass}`} style={{color:'#6B7280'}}>{i.names[lang]} ×{i.qty}</span>
                  <span className="font-black flex-shrink-0" style={{color:'#1A2F23'}}>{i.price*i.qty} MAD</span>
                </div>
              ))}
              <div className="flex justify-between text-sm mt-2 pt-2" style={{borderTop:'1px solid #E5E1D8'}}>
                <span className={`font-black ${fClass}`} style={{color:'#065F46'}}>{t.total}</span>
...
function HomePage({lang,t,onAddToCart}:{lang:Lang;t:typeof T.fr;onAddToCart:(i:MenuItem)=>void}) {
    ...
    <div>
      ...
      <section className="relative mx-5 mb-6 rounded-3xl overflow-hidden shadow-xl group">
        ...
        <div className="absolute bottom-0 left-0 right-0 p-5">
          ...
          <button onClick={()=>document.getElementById('menu-section')?.scrollIntoView({behavior:'smooth',block:'start'})}
      ...
      <section id="menu-section" className="mb-6" style={{scrollMarginTop:'80px'}}>
        <div className="px-5 mb-3 flex items-center gap-2">
          ...
          <p className={`text-[11px] font-black uppercase tracking-widest ${fClass}`} style={{color:'#065F46'}}>{t.menuTitle}</p>
        </div>
        <div className="flex gap-2 px-5 mb-4 overflow-x-auto hide-scrollbar pb-1" style={{direction:isAR?'rtl':'ltr'}}>
          {CATEGORIES.map(cat=>{const active=activeCat===cat.id;return(
            <button key={cat.id} onClick={()=>setActiveCat(cat.id)}
              className={`flex-shrink-0 flex items-center gap-1.5 px-3.5 py-2 rounded-2xl font-black text-[11px] transition-all active:scale-95 ${fontClass(lang)==='font-tifinagh'?'font-tifinagh':fontClass(lang)==='font-arabic'?'font-arabic':''}`}
              ...
              <span>{cat.emoji}</span><span>{cat.labels[lang]}</span>
            </button>
          );})}
        </div>
        <div className="px-5 grid grid-cols-2 gap-3">
          {filtered.map(item=><FoodCard key={item.id} item={item} lang={lang} onAdd={onAddToCart} onView={setModalItem}/>)}
        </div>
      </section>
      ...
      <section className="px-5 mb-6">
        <div className="flex items-center gap-2 mb-3">
          ...
          <p className={`text-[11px] font-black uppercase tracking-widest ${fClass}`} style={{color:'#B45309'}}>{t.payment}</p>
        </div>
        <div className="grid grid-cols-2 gap-3">
          {[{key:'cash'as const,icon:'🤝',label:t.cashLabel,desc:t.cashDesc},{key:'card'as const,icon:'💳',label:t.cardLabel,desc:t.cardDesc}].map(({key,icon,label,desc})=>(
            <button key={key} onClick={()=>setSelectedPay(key)} className="p-4 rounded-2xl flex flex-col items-center text-center transition-all active:scale-95"
              style={{background:selectedPay===key?'#F0FDF4':'#FDFCF9',border:`2px solid ${selectedPay===key?'#065F46':'#E5E1D8'}`}}>
              <div className="w-11 h-11 rounded-full flex items-center justify-center text-xl mb-2" style={{background:selectedPay===key?'#D1FAE5':'#F0EDE7'}}>{icon}</div>
              ...
              {selectedPay===key&&<span className={`mt-1.5 text-[9px] font-black ${fClass}`} style={{color:'#065F46'}}>{t.selected}</span>}
            </button>
          ))}
        </div>
      </section>
      ...
      <section className="px-5 mb-6">
        <div className="rounded-3xl overflow-hidden" style={{border:'1.5px solid #E5E1D8',background:'#FDFCF9'}}>
          ...
          <div className="px-4 pb-2 flex items-center gap-3">
            ...
            <div className="flex items-center gap-1"><span className="text-yellow-400 text-xs">★</span><span className="text-xs font-bold" style={{color:'#1A2F23'}}>{t.courierRating}</span></div>
          </div>
      ...
      {modalItem&&<QuickViewModal item={modalItem} lang={lang} onClose={()=>setModalItem(null)} onAdd={onAddToCart}/>}
    </div>
  );
}
    ...
    <div className="px-5">
      ...
      {[
        ...
        <a key={i} href={item.href} target={item.href.startsWith('http')?'_blank':undefined} rel="noopener noreferrer"
          ...
          <div className="w-12 h-12 rounded-full flex items-center justify-center flex-shrink-0" style={{background:item.iconBg,border:item.iconBorder?`2px solid ${item.iconBorder}`:undefined}}>{item.icon}</div>
          <div className="flex-1 min-w-0">
...
export default function App() {
  ...
  const adjustQty=(id:number,delta:number)=>setCart(prev=>
    prev.flatMap(i=>{if(i.id!==id)return[i];const q=i.qty+delta;return q>0?[{...i,qty:q}]:[];})
  );
    ...
    <div className={`min-h-screen overflow-x-hidden ${isAR?'rtl':'ltr'}`} style={{color:'#1A2F23'}}>
      ...
      <div className={`fixed top-5 z-50 flex items-center gap-2 ${isAR?'right-5':'left-5'}`}>
        ...
        <button onClick={()=>setShowProfile(true)}
          className="h-11 w-11 rounded-full flex items-center justify-center font-black text-sm transition-all active:scale-90 hover:scale-110 relative"
          style={{background:'white',border:'2.5px solid #D9C5A0',color:'#065F46',boxShadow:'0 4px 20px rgba(6,95,70,0.15)'}}>
          👤
          {profile.name&&<span className="absolute -top-1 -right-1 w-3 h-3 rounded-full bg-emerald-500 border-2 border-white"/>}
        </button>
      </div>
      ...
      <nav className="fixed bottom-0 inset-x-0 z-40"
        ...
        <div className="max-w-md mx-auto flex">
          {([{id:'home',label:t.navHome,icon:'🏠'},{id:'tracking',label:t.navTrack,icon:'📍'},{id:'contact',label:t.navContact,icon:'💬'}] as {id:Page;label:string;icon:string}[]).map(tab=>(
            <button key={tab.id} onClick={()=>setPage(tab.id)} className="flex-1 flex flex-col items-center gap-1 py-3 transition-all">
              <span className="text-xl transition-transform" style={{transform:page===tab.id?'scale(1.15)':'scale(1)'}}>{tab.icon}</span>
              <span className={`text-[10px] font-black uppercase tracking-wide ${fClass}`} style={{color:page===tab.id?'#065F46':'#9CA3AF'}}>{tab.label}</span>
      ...
      {/* Checkout drawer */}
      {showCart&&(
        <CheckoutDrawer cart={cart} lang={lang} onClose={()=>setShowCart(false)} onQty={adjustQty} profile={profile} onClearCart={clearCart}/>
      )}

      {/* Profile modal */}
      {showProfile&&(
        <ProfileModal lang={lang} profile={profile} onSave={saveProfile} onClose={()=>setShowProfile(false)}/>
      )}
    </div>
  );
}
