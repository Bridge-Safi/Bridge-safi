...
function AdminOrdersPanel({ adminKey }: { adminKey: string }) {
  ...
  const [loading, setLoading] = useState(false);
  // { [ref]: string } — texte saisi dans le champ livreur par commande
  ...
  const assign = async (ref: string) => {
    const name = (driverInputs[ref] || '').trim();
    if (!name) return;
    setSaving(p => ({ ...p, [ref]: true }));
    try {
      const r = await fetch(`/api/admin/orders/${encodeURIComponent(ref)}/assign`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ adminKey: adminKey.trim(), driverName: name }),
      });
      if (r.ok) {
        setSaved(p => ({ ...p, [ref]: true }));
        setTimeout(() => setSaved(p => ({ ...p, [ref]: false })), 2500);
        // Met à jour l'ordre local
        setOrders(prev => prev.map(o => o.ref === ref ? { ...o, driverName: name } : o));
      }
    } finally { setSaving(p => ({ ...p, [ref]: false })); }
  };
    ...
    <div style={{ marginTop: 24, padding: 16, background: '#0F172A', borderRadius: 14, border: '1px solid #1E3A5F' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
        <h3 style={{ fontSize: 14, fontWeight: 900, color: '#F8FAFC', margin: 0 }}>
          📋 Commandes en cours
          {orders.length > 0 && (
            <span style={{ marginLeft: 8, fontSize: 11, background: '#1E3A5F', color: '#93C5FD', borderRadius: 20, padding: '2px 8px', fontWeight: 700 }}>
              {orders.length}
            </span>
          )}
        </h3>
        <button onClick={refresh} disabled={loading}
          style={{ fontSize: 11, fontWeight: 800, color: '#93C5FD', background: 'transparent', border: '1px solid #1E3A5F', borderRadius: 8, padding: '4px 10px', cursor: 'pointer' }}>
          {loading ? '…' : '🔄'}
        </button>
      </div>
      ...
      {pending.length > 0 && (
        <div style={{ marginBottom: 10 }}>
          <div style={{ fontSize: 10, fontWeight: 900, color: '#F59E0B', letterSpacing: '0.1em', marginBottom: 6 }}>
            🟡 EN ATTENTE ({pending.length})
          </div>
          {pending.map(o => <AdminOrderCard key={o.ref} o={o} adminKey={adminKey}
            driverInput={driverInputs[o.ref] || ''} saving={!!saving[o.ref]} saved={!!saved[o.ref]}
            onDriverInput={v => setDriverInputs(p => ({ ...p, [o.ref]: v }))}
            onAssign={() => assign(o.ref)} />)}
        </div>
      )}
      ...
      {active.length > 0 && (
        <div>
          <div style={{ fontSize: 10, fontWeight: 900, color: '#60A5FA', letterSpacing: '0.1em', marginBottom: 6 }}>
            🛵 EN COURS ({active.length})
          </div>
          {active.map(o => <AdminOrderCard key={o.ref} o={o} adminKey={adminKey}
            driverInput={driverInputs[o.ref] || ''} saving={!!saving[o.ref]} saved={!!saved[o.ref]}
            onDriverInput={v => setDriverInputs(p => ({ ...p, [o.ref]: v }))}
            onAssign={() => assign(o.ref)} />)}
        </div>
...
function AdminOrderCard({ o, adminKey: _adminKey, driverInput, saving, saved, onDriverInput, onAssign }:
  { o: any; adminKey: string; driverInput: string; saving: boolean; saved: boolean;
    onDriverInput: (v: string) => void; onAssign: () => void }) {
  const st = STATUS_LABEL[o.status] ?? { label: o.status, color: '#9CA3AF' };
  const time = new Date(o.createdAt).toLocaleTimeString('fr-FR', { hour: '2-digit', minute: '2-digit' });
  const items: { name: string; qty: number; price: number }[] = Array.isArray(o.items) ? o.items : [];

  return (
    <div style={{ background: '#1E293B', borderRadius: 12, padding: '12px 14px', marginBottom: 8,
      border: `1px solid ${o.status === 'pending' ? '#78350F' : '#1E3A5F'}` }}>

      {/* Ref + statut + heure */}
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <span style={{ fontSize: 11, fontWeight: 900, color: '#D9C5A0', letterSpacing: '0.08em' }}>{o.ref}</span>
          <span style={{ fontSize: 9, color: '#64748B', fontWeight: 700 }}>{time}</span>
        </div>
        <span style={{ fontSize: 10, fontWeight: 800, color: st.color, background: `${st.color}22`,
          padding: '2px 8px', borderRadius: 8 }}>{st.label}</span>
      </div>

      {/* Client */}
      <p style={{ margin: '0 0 2px', fontSize: 12, fontWeight: 700, color: '#F1F5F9' }}>
      ...
      {/* Articles */}
      {items.length > 0 && (
        <div style={{ background: '#0F172A', borderRadius: 8, padding: '6px 10px', marginBottom: 8 }}>
          {items.map((it, i) => (
            <span key={i} style={{ fontSize: 10, color: '#94A3B8', display: 'block' }}>
              ×{it.qty} {it.name} — {it.price} MAD
            </span>
          ))}
        </div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
        <span style={{ fontSize: 13, fontWeight: 900, color: '#4ADE80' }}>💰 {o.total} MAD</span>
        <span style={{ fontSize: 10, color: '#475569', fontWeight: 700 }}>{o.service || o.restaurantName || ''}</span>
      </div>

      {/* Livreur assigné ou champ d'assignation */}
      {o.driverName ? (
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '7px 10px',
          background: '#064E3B', borderRadius: 8, border: '1px solid #065F46' }}>
          <span style={{ fontSize: 12, fontWeight: 900, color: '#4ADE80' }}>🛵 {o.driverName}</span>
          <button onClick={() => onDriverInput('')}
            style={{ marginLeft: 'auto', fontSize: 10, color: '#6EE7B7', background: 'transparent',
        ...
        <div style={{ display: 'flex', gap: 6 }}>
          <input
            value={driverInput}
            onChange={e => onDriverInput(e.target.value)}
            onKeyDown={e => { if (e.key === 'Enter') onAssign(); }}
            placeholder="Nom du livreur…"
            style={{ flex: 1, padding: '7px 10px', borderRadius: 8, border: '1px solid #334155',
              background: '#0F172A', color: '#F1F5F9', fontSize: 12, fontWeight: 700, outline: 'none' }}
          />
          <button onClick={onAssign} disabled={saving || !driverInput.trim()}
            style={{ padding: '7px 14px', borderRadius: 8, border: 'none', cursor: saving ? 'default' : 'pointer',
              background: saved ? '#065F46' : '#1D4ED8', color: '#fff', fontSize: 12, fontWeight: 900,
              opacity: saving || !driverInput.trim() ? 0.6 : 1 }}>
            {saved ? '✅' : saving ? '…' : 'Assigner'}
          </button>
        </div>
      )}
    </div>
